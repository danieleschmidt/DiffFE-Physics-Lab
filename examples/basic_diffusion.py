...
def solve_basic_diffusion():
    ...
    if not HAS_FIREDRAKE:
        ...
        try:
            ...
            logger.info(
                f"Manufactured solution at (0.5, 0.5): u = {u_exact:.4f}, f = {f_source:.4f}"
            )
        ...
        try:
            ...
            logger.info(
                f"Operator MMS at (0.25, 0.75): u = {u_val:.4f}, f = {f_val:.4f}"
            )
    ...
    problem.add_boundary_condition(
        bc_type="dirichlet", boundary_id="on_boundary", value=0.0
    )
...
def solve_with_manufactured_solution():
    ...
    problem = FEBMLProblem(mesh=mesh, function_space=V, experiment_name="mms_diffusion")
    ...
    problem.add_boundary_condition(
        bc_type="dirichlet", boundary_id="on_boundary", value=mms["solution"]
    )
...
def parameter_optimization_example():
    ...
    np.random.seed(42)
    observations = true_sol.dat.data + 0.01 * np.random.normal(
        size=len(true_sol.dat.data)
    )
...
def convergence_study():
    ...
    for i, (h, ndof, err) in enumerate(zip(h_values, dofs, errors)):
        if i == 0:
            ...
            rate = np.log(errors[i] / errors[i - 1]) / np.log(
