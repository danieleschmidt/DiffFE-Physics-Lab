...
def main():
    ...
    for test_size in [10000, 50000, 100000]:
        ...
        print(f"   📊 Size {test_size:6d}: Pool={pool_time:.3f}s, Regular={regular_time:.3f}s, Speedup={speedup:.2f}x")
    
    # Memory statistics
    mem_stats = memory_optimizer.get_memory_stats()
    print(f"   📈 Memory stats: RSS={mem_stats['rss_mb']:.1f}MB, Usage={mem_stats['percent']:.1f}%")
    ...
    try:
        ...
        while completed_tasks < len(task_ids) and time.time() - start_time < 30:
            ...
            if result:
                completed_tasks += 1
                if result['success']:
                    print(f"   ✅ Task {result['task_id'][-4:]}: {result['result']:.3f} "
        ...
        print(f"   📊 Final metrics: {final_metrics.active_workers} workers, "
    ...
    for hour in range(24):
        ...
        print(f"   🕐 Hour {hour:2d}: Load={load:5.1f}%, "
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for domain_name, future in futures:
                try:
                    ...
                    print(f"     ✅ {domain_name}: T={result['temperature']:.1f}°C, "
    ...
    print(f"   🚀 Starting stress test: {num_workers} workers, {operations_per_worker} ops each")
    ...
    print(f"     - Error rate: {(total_errors / (total_ops + total_errors) * 100):.2f}%")
