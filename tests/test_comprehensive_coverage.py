...
from src.utils.error_computation import compute_h1_error, compute_l2_error
...
from src.utils.validation import validate_boundary_conditions, validate_function_space
...
class TestUtilities:
    ...
    def test_validation_functions(self):
        ...
        bc_dict = {"test_bc": {"type": "dirichlet", "boundary": 1, "value": 0.0}}
...
class TestPropertyBased:
    ...
    def test_gradient_accuracy(self):
        ...
        grad_fd = np.zeros(2)
        grad_fd[0] = (
            test_function([x[0] + h, x[1]]) - test_function([x[0] - h, x[1]])
        ) / (2 * h)
        grad_fd[1] = (
            test_function([x[0], x[1] + h]) - test_function([x[0], x[1] - h])
        ) / (2 * h)
