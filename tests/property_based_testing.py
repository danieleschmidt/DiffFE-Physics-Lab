...
class StabilityProperty(NumericalProperty):
    ...
    def test_property(
        ...
        try:
            ...
            if original_value != 0:
                relative_change = abs(perturbed_value - original_value) / abs(original_value)
        ...
        try:
            ...
            for step_size in step_sizes:
                ...
                for key in initial_params.keys():
                    current_params[key] = initial_params[key] + step_size * step_direction[key]
...
class ScaleInvarianceProperty(NumericalProperty):
    ...
    def test_property(
        ...
        try:
            ...
            # Scaled problem
            scaled_params = {key: value * scale_factor for key, value in parameters.items()}
...
class PropertyBasedTester:
    ...
    def test_solver_convergence_properties(self):
        ...
        def test_convergence_property(problem_size, tolerance, max_iterations, backend):
            ...
            assert result, f"Convergence property failed for size={problem_size}, tol={tolerance}"
    ...
    def test_scale_invariance_properties(self):
        ...
        def test_scale_property(parameters, scale_factor):
            ...
            assert result, f"Scale invariance failed for params={parameters}, scale={scale_factor}"
    ...
    def _generate_recommendations(self, property_stats: Dict[str, Any]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("All property tests are passing with good performance")
    ...
    class OptimizationStateMachine(RuleBasedStateMachine):
        ...
        @rule(
            param_update=st.dictionaries(
                ...
                values=st.floats(min_value=-5.0, max_value=5.0, allow_nan=False, allow_infinity=False),
                min_size=1,
                max_size=2
            )
        )
        ...
        def objective_is_finite(self):
            ...
            try:
                obj_value = self.objective(self.current_parameters)
                assert np.isfinite(obj_value), f"Objective returned non-finite value: {obj_value}"
            except Exception as e:
...
class RegressionTester:
    ...
    def test_regression(self, test_case_name: str, current_result: Any) -> bool:
        ...
        try:
            if isinstance(reference, (int, float)) and isinstance(current_result, (int, float)):
                # Scalar comparison
                if not np.isfinite(reference) or not np.isfinite(current_result):
                    return np.isfinite(reference) == np.isfinite(current_result)
                
                relative_error = abs(current_result - reference) / max(abs(reference), 1e-10)
                ...
                if not np.all(np.isfinite(ref_array)) or not np.all(np.isfinite(cur_array)):
                    return np.array_equal(np.isfinite(ref_array), np.isfinite(cur_array))
                
                relative_errors = np.abs(cur_array - ref_array) / np.maximum(np.abs(ref_array), 1e-10)
