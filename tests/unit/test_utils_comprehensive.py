...
from src.utils.exceptions import ConvergenceError, SolverError
...
try:
    from src.utils.error_computation import compute_convergence_rate, compute_error
...
class TestConfigManager:
    ...
    def test_config_manager_update(self):
        ...
        update_data = {
            "backend": "torch",
            "solver": {"tolerance": 1e-10, "method": "newton"},
        }
    ...
    def test_config_manager_validate(self):
        ...
        # Valid configuration
        valid_config = {"backend": "jax", "tolerance": 1e-6, "max_iterations": 100}
    ...
    def test_config_manager_environment_override(self):
        ...
        with patch.dict(
            os.environ, {"DIFFFE_BACKEND": "torch", "DIFFFE_TOLERANCE": "1e-8"}
        ):
            config_manager = ConfigManager()
...
class TestExceptions:
    ...
    def test_solver_error_with_details(self):
        ...
        details = {"iterations": 1000, "residual": 1e-3, "tolerance": 1e-6}
...
class TestManufacturedSolutions:
    ...
    def test_generate_polynomial_solution(self):
        ...
        solution = generate_manufactured_solution(
            SolutionType.POLYNOMIAL, dimension=2, parameters={"degree": 2}
        )
    ...
    def test_generate_exponential_solution(self):
        ...
        solution = generate_manufactured_solution(
            SolutionType.EXPONENTIAL, dimension=2, parameters={"decay_rate": 1.0}
        )
    ...
    def test_manufactured_solution_different_dimensions(self):
        ...
        solution_1d = generate_manufactured_solution(
            SolutionType.TRIGONOMETRIC, dimension=1, parameters={"frequency": 2.0}
        )
        ...
        solution_3d = generate_manufactured_solution(
            SolutionType.TRIGONOMETRIC, dimension=3, parameters={"frequency": 1.0}
        )
    ...
    def test_manufactured_solution_parameter_validation(self):
        ...
        with pytest.raises(ValueError, match="Dimension must be positive"):
            generate_manufactured_solution(SolutionType.TRIGONOMETRIC, dimension=0)

        # Invalid solution type
        with pytest.raises(ValueError, match="Unsupported solution type"):
            generate_manufactured_solution("invalid_type", dimension=2)

        # Invalid frequency
        with pytest.raises(ValueError):
            generate_manufactured_solution(
                SolutionType.TRIGONOMETRIC, dimension=2, parameters={"frequency": -1.0}
            )
...
@pytest.mark.skipif(
    not HAS_ERROR_COMPUTATION, reason="Error computation module not available"
)
class TestErrorComputation:
    ...
    def test_compute_l2_error_simple(self):
        ...
        l2_error = compute_error(
            approximate_solution, exact_solution, "L2", points=points
        )
        assert l2_error > 0
