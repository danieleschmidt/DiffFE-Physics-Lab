...
from src.security.monitor import (
    RateLimitRule,
    SecurityEvent,
    SecurityMonitor,
    ThreatLevel,
)
from src.security.scanner import SecurityIssue, SecurityScanner, Severity
...
"""
    ...
    def test_generate_report(self):
        ...
        scanner.issues = [
            ...
            SecurityIssue(
                "sql_injection", Severity.HIGH, "SQL injection risk", "file2.py", 25
            ),
            SecurityIssue(
                "weak_crypto", Severity.MEDIUM, "Weak encryption", "file3.py", 15
            ),
        ]
...
class TestSecurityValidator:
    ...
    def test_validate_file_upload(self):
        ...
        # Invalid extension
        invalid_ext_result = validator.validate_file_upload(
            "script.exe", b"MZ\x90\x00", 1024
        )
        assert invalid_ext_result.is_valid is False

        # File too large
        large_file_result = validator.validate_file_upload(
            "large.pdf", b"%PDF-1.4", 50 * 1024 * 1024
        )
        assert large_file_result.is_valid is False

        # Suspicious content
        suspicious_result = validator.validate_file_upload(
            "doc.pdf", b"<script>alert()</script>", 1024
        )
...
class TestSecurityMonitor:
    ...
    def test_rate_limiting(self):
        ...
        monitor.add_rate_limit_rule(
            rule_name="test_api", max_requests=2, time_window=60, block_duration=300
        )
        ...
        rate_limit_events = [
            e for e in monitor.events if e.event_type == "rate_limit_exceeded"
        ]
    ...
    def test_brute_force_detection(self):
        ...
        brute_force_events = [
            e for e in monitor.events if e.event_type == "brute_force_attack"
        ]
    ...
    def test_injection_attack_detection(self):
        ...
        injection_events = [
            e for e in monitor.events if e.event_type == "injection_attack"
        ]
    ...
    def test_get_security_metrics(self):
        ...
        # Add some test data
        monitor.record_event(
            "test_event", ThreatLevel.MEDIUM, "Test message", "192.168.1.100"
        )
        monitor.blocked_ips["192.168.1.100"] = (
            time.time() + 300
    ...
    def test_get_recent_events(self):
        ...
        monitor.record_event(
            "event1", ThreatLevel.MEDIUM, "Medium threat", "192.168.1.102"
        )
    ...
    def test_generate_security_report(self):
        ...
        monitor.record_event(
            "login_failed", ThreatLevel.MEDIUM, "Failed login", "192.168.1.100"
        )
        monitor.record_event(
            "sql_injection", ThreatLevel.HIGH, "SQL injection", "192.168.1.100"
        )
        monitor.record_event(
            "login_failed", ThreatLevel.MEDIUM, "Failed login", "192.168.1.101"
        )
...
"""
        ...
        injection_events = [
            e for e in monitor.events if e.event_type == "sql_injection"
        ]
        assert len(injection_events) == 1
