...
from src.backends.base import ADBackend, get_backend, list_backends, set_default_backend
...
class TestJAXBackend:
    ...
    @pytest.mark.parametrize(
        "input_data",
        [[1, 2, 3], np.array([1.0, 2.0, 3.0]), ((1, 2), (3, 4)), {"a": 1, "b": 2}],
    )
...
class TestTorchBackend:
    ...
    def torch_backend(self):
        ...
        with patch("src.backends.torch_backend.HAS_TORCH", True):
            with patch("src.backends.torch_backend.torch") as mock_torch:
                ...
                # Mock autograd
                mock_torch.autograd.functional.jacobian = Mock(
    ...
    def test_torch_array_creation(self, torch_backend):
        ...
        torch_backend.torch.tensor.assert_called_once_with(
            data, dtype=torch_backend.torch.float32
        )
...
class TestBackendInteroperability:
    ...
    @pytest.mark.parametrize(
        "backend_name,expected_type", [("jax", "JAXBackend"), ("torch", "TorchBackend")]
    )
        ...
        with patch("src.backends.jax_backend.HAS_JAX", True):
            with patch("src.backends.jax_backend.jax") as mock_jax:
                with patch("src.backends.jax_backend.jnp"):
                    ...
                    mock_jax.grad.side_effect = ValueError(
