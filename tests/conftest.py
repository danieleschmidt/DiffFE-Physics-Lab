...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(
            ...
            for keyword in ["convergence", "optimization", "benchmark"]
        ):
...
def sample_data():
    ...
    return {
        ...
        "dimension": 2,
        "dofs": 1000,
        "test_points": np.array([[0.1, 0.1], [0.5, 0.5], [0.9, 0.9]]),
    }
...
def mock_firedrake():
    ...
    mock_fd.FunctionSpace = Mock(
        return_value=Mock(
            dim=Mock(return_value=121), mesh=Mock(return_value=mock_fd.UnitSquareMesh())
        )
    )
...
def simple_problem_config():
    ...
    return {
        "mesh": {"type": "unit_square", "n_elements": 16},
        "function_space": {"family": "CG", "degree": 1},
        "equation": {"type": "laplacian", "parameters": {"diffusion_coeff": 1.0}},
        "boundary_conditions": {"dirichlet": {"boundary": "on_boundary", "value": 0.0}},
...
if HAS_FLASK:
    ...
    def flask_app():
        ...
        app = create_app(
            {"TESTING": True, "DEBUG": True, "SECRET_KEY": "test-secret-key"}
        )
...
def mms_polynomial_2d():
    ...
    return generate_manufactured_solution(
        solution_type=SolutionType.POLYNOMIAL, dimension=2, parameters={"degree": 2}
    )
...
def assert_almost_equal():
    ...
    def _assert_almost_equal(actual, expected, rtol=1e-7, atol=1e-12, msg=""):
        if np.isscalar(actual) and np.isscalar(expected):
            assert abs(actual - expected) <= atol + rtol * abs(
                expected
...
def assert_solution_accuracy(
    computed_solution, exact_solution, expected_error, tolerance=0.1
):
