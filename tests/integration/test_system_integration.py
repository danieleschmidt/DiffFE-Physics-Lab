...
from src.performance import PerformanceMonitor, PerformanceOptimizer
    ...
    def test_problem_optimization_integration(self, mock_fd):
        ...
        with patch("scipy.optimize.minimize") as mock_minimize:
            ...
            result = opt_service.minimize_vector(
                objective=objective, initial_guess=np.array([1.0])
            )
...
class TestPerformanceIntegration:
    ...
    def test_performance_profiling_integration(self):
        ...
        from src.performance import get_global_profiler, profile
...
class TestSecurityIntegration:
    ...
    def test_security_monitoring_integration(self):
        ...
        monitor.add_rate_limit_rule(
            rule_name="api_test", max_requests=3, time_window=60, block_duration=300
        )
...
class TestFullSystemIntegration:
    ...
    def test_complete_system_workflow(self, mock_fd):
        ...
        from src.performance import get_global_optimizer, get_global_profiler
        from src.security.monitor import SecurityMonitor
        ...
        with patch("scipy.optimize.minimize") as mock_minimize:
            ...
            opt_result = opt_service.minimize_vector(
                objective=secure_objective, initial_guess=np.array([1.0])
            )
        ...
        critical_events = [
            e
            for e in security_monitor.events
    ...
    def test_cascading_error_handling(self):
        ...
        try:
            ...
            error_handler.log_error(
                ErrorLevel.ERROR, str(e), {"component": "validation"}
            )
        ...
        try:
            ...
            error_handler.log_error(
                ErrorLevel.CRITICAL, str(e), {"component": "solver"}
            )
        ...
        high_threat_events = [
            e for e in security_monitor.events if e.threat_level == ThreatLevel.HIGH
        ]
        ...
        combined_report = {
            ...
            "correlation": {
                "total_incidents": len(error_handler.errors)
                + len(security_monitor.events),
                "critical_level": error_handler.has_critical_errors()
                or len(high_threat_events) > 0,
            },
        }
