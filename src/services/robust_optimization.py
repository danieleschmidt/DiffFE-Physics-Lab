...
from ..utils.exceptions import OptimizationError, ValidationError, ErrorCode
...
class ProgressReporter:
        ...
        if len(self.objective_history) > 0:
            ...
            if progress.total_iterations:
                ...
                if self.iteration_times:
                    avg_iteration_time = np.mean(self.iteration_times[-20:])  # Use recent average
                    progress.estimated_remaining_time = remaining_iterations * avg_iteration_time
    ...
    def _log_progress(self, progress: OptimizationProgress):
        ...
        if progress.status == 'converged':
            ...
            logger.warning(f"⏸ Optimization stopped: {log_message} - {progress.message}")
        else:
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str, optimization_id: str = None):
        ...
        self.checkpoint_file = self.checkpoint_dir / f"{self.optimization_id}_checkpoint.pkl"
        self.metadata_file = self.checkpoint_dir / f"{self.optimization_id}_metadata.json"
    ...
    def list_checkpoints(self) -> List[Dict[str, Any]]:
        ...
        for metadata_file in self.checkpoint_dir.glob("*_metadata.json"):
            try:
                ...
                logger.warning(f"Error reading checkpoint metadata {metadata_file}: {e}")
...
class RobustOptimizer:
    ...
    def optimize(
        ...
        if resume_from_checkpoint and self.checkpoint_manager:
            ...
            if checkpoint:
                ...
                logger.info(f"Resuming optimization from iteration {checkpoint.iteration}")
        
        try:
            with self._optimization_context():
                result = self._run_optimization(objective, start_params, bounds, constraints)
    ...
    def _setup_optimization_session(self, initial_params: Dict[str, Any], bounds: Optional[Dict]):
        """Setup optimization session."""
        # Setup checkpoint manager
        if self.config.enable_checkpointing:
            checkpoint_dir = self.config.checkpoint_directory or "./checkpoints"
            self.checkpoint_manager = CheckpointManager(checkpoint_dir, self.optimization_id)
    ...
    def _wrap_objective_function(self, objective: Callable, param_names: List[str]) -> Callable:
        ...
        def wrapped_objective(x: np.ndarray) -> float:
            ...
            if self.config.timeout_hours:
                if self.total_time > self.config.timeout_hours * 3600:
                    raise TimeoutError(f"Optimization timeout after {self.config.timeout_hours} hours")
            ...
            try:
                ...
                # Validate result
                if not np.isfinite(value):
                    logger.warning(f"Objective function returned non-finite value: {value}")
                    return np.inf
    ...
    def _wrap_gradient_function(self, objective: Callable, param_names: List[str]) -> Callable:
            ...
            try:
                if grad_func:
                    ...
                    with PerformanceLogger("finite_difference_gradient", logger, 0.0):
                        gradient = self._compute_finite_difference_gradient(objective, params, param_names)
    ...
    def _create_optimization_callback(self, param_names: List[str]) -> Callable:
        ...
        def callback(x: np.ndarray, **kwargs):
            ...
            checkpoint = OptimizationCheckpoint(
                ...
                cumulative_time=time.time() - self.progress_reporter.start_time if self.progress_reporter else 0.0,
                convergence_history=[],
                metadata={
                    'method': self.config.method,
                    'backend': self.actual_backend
            ...
            if (self.config.enable_checkpointing and 
                ...
                iteration % self.config.checkpoint_frequency == 0):
    ...
    def _create_partial_result(self, x: np.ndarray, param_names: List[str], message: str) -> OptimizeResult:
