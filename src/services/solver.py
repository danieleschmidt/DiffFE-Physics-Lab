...
class FEBMLSolver:
    ...
    def compute_sensitivities(
        self, output_functional: Callable, parameters: List[str]
    ) -> Dict[str, np.ndarray]:
    ...
    def verify_convergence(
        self, exact_solution: Callable = None, refinement_levels: int = 3
    ) -> Dict[str, Any]:
        ...
        for level in range(refinement_levels):
            ...
            if level > 0:
                ...
                self.problem.function_space = fd.FunctionSpace(
                    self.problem.mesh, self.problem.function_space.ufl_element()
                )
            ...
            logger.info(
                f"Level {level}: h={h:.3e}, DOFs={ndofs}, "
                f"error={results['errors'][-1]:.3e}"
                if results["errors"]
                else ""
            )
    ...
    def benchmark_performance(self, num_runs: int = 5) -> Dict[str, Any]:
        ...
        logger.info(
            f"Benchmark complete: {results['mean_time']:.3f}±{results['std_time']:.3f}s, "
