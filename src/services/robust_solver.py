...
from ..utils.exceptions import (
    SolverError, ConvergenceError, ResourceError,
    handle_solver_failure, ErrorCode
)
from ..utils.logging_config import get_logger, PerformanceLogger, log_performance
...
class RobustFEBMLSolver:
    ...
    def solve(
        ...
        if prob is None:
            raise SolverError("No problem provided", error_code=ErrorCode.SOLVER_SETUP_ERROR)
        ...
        try:
            with self._memory_limit_context(), self._timeout_context() as check_timeout:
                solution = self._solve_with_retry(prob, parameters, metrics, check_timeout)
            ...
            monitor.log_application_metric('convergence_iterations', metrics.convergence_iterations)
    ...
    def _solve_with_retry(
        ...
        while retry_count <= self.retry_config.max_retries:
            try:
                logger.debug(f"Solver attempt {retry_count + 1}/{self.retry_config.max_retries + 1}")
                
                # Adjust parameters for retry
                if retry_count > 0:
                    solve_params = self._adapt_parameters_for_retry(solve_params, retry_count, last_exception)
                
                # Attempt solve
                solution = self._solve_attempt(problem, solve_params, metrics, check_timeout)
                ...
                if retry_count <= self.retry_config.max_retries:
                    ...
                    logger.warning(f"Solver attempt {retry_count} failed: {e}. Retrying in {delay:.1f}s...")
                    time.sleep(delay)
        
        # All retries exhausted
        logger.error(f"Solver failed after {self.retry_config.max_retries + 1} attempts")
    ...
    def _solve_attempt(
        ...
        if is_linear:
            solution = self._solve_linear_monitored(problem, u, v, parameters, metrics, check_timeout)
        else:
            solution = self._solve_nonlinear_monitored(problem, u, v, parameters, metrics, check_timeout)
        ...
        logger.info(f"FEM solve completed - DOFs: {metrics.dofs}, Time: {metrics.solve_time:.3f}s")
    ...
    def _solve_nonlinear_monitored(
        ...
        for iteration in range(max_iter):
            ...
            if adaptive_tol and iteration > 5:
                ...
                convergence_rate = residuals[-1] / residuals[-2] if len(residuals) > 1 else 1.0
    ...
    def _should_retry(self, exception: Exception, attempt: int) -> bool:
        ...
        if isinstance(exception, ConvergenceError) and self.retry_config.retry_on_convergence_failure:
            return True
        
        if isinstance(exception, (MemoryError, ResourceError)) and self.retry_config.retry_on_memory_error:
            return True
        
        # Check for numerical errors
        if self.retry_config.retry_on_numerical_error:
            error_msg = str(exception).lower()
            numerical_indicators = ['nan', 'inf', 'singular', 'numerical', 'overflow', 'underflow']
    ...
    def _adapt_parameters_for_retry(
        ...
        if isinstance(last_exception, ConvergenceError):
            ...
            if 'tolerance' in adapted_params:
                adapted_params['tolerance'] = min(adapted_params['tolerance'] * 10, 1e-4)
            
            if 'max_iterations' in adapted_params:
                adapted_params['max_iterations'] = min(adapted_params['max_iterations'] * 2, 1000)
    ...
    def _get_nonlinear_solver_params(self, iteration: int) -> Dict[str, Any]:
        ...
        base_params = {
            'ksp_type': 'preonly' if iteration < 5 else self.options.get('linear_solver', 'gmres'),
            'pc_type': 'lu' if iteration < 5 else self.options.get('preconditioner', 'ilu'),
            'ksp_rtol': 1e-12,
            'ksp_atol': 1e-14,
            'ksp_max_it': 1000,
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            'last_solve_success': self.last_metrics.success if self.last_metrics else None,
        ...
        try:
            ...
            if self.metrics_history:
                ...
                if avg_solve_time > 300:  # More than 5 minutes average
                    ...
                    health['issues'].append(f'Slow average solve time: {avg_solve_time:.1f}s')
                ...
                if failure_rate > 0.2:  # More than 20% failures
                    ...
                    health['issues'].append(f'High failure rate: {failure_rate*100:.1f}%')
    ...
    def get_performance_metrics(self, last_n: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_convergence_iterations': np.mean([m.convergence_iterations for m in recent_metrics]),
