...
def register_error_handlers(app):
    ...
    def method_not_allowed(error):
        ...
        return jsonify({
            ...
            'status_code': 405,
            'allowed_methods': error.description.get('valid_methods', []) if hasattr(error, 'description') else []
        }), 405
    ...
    def unsupported_media_type(error):
        ...
        logger.warning(f"Unsupported media type: {request.method} {request.path} - {request.content_type}")
        return jsonify({
    ...
    def handle_generic_exception(error):
        ...
        logger.error(f"Unhandled exception: {request.method} {request.path} - {type(error).__name__}: {error}")
