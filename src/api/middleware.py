...
def setup_middleware(app):
    ...
    def before_request():
        ...
        request.environ["REQUEST_TIME"] = time.strftime(
    ...
    def validate_request():
        ...
        if (
            ...
            and request.path not in ["/api/info", "/api/operators"]
        ):
    ...
    def log_request_info(exception):
        if exception:
            logger.error(
                f"Request failed: {request.method} {request.path} - {exception}"
            )
...
def log_performance(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            end_time = time.time()
            logger.error(
                f"Performance: {f.__name__} failed after {end_time - start_time:.3f}s: {e}"
            )
            raise
