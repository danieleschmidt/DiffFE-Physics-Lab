    ...
    def _check_backends_health(self, include_details: bool = True) -> HealthStatus:
        ...
        try:
            ...
            if not available_backends:
                ...
                status = "degraded"
                message = (
                    f"Only fallback backends available: {', '.join(available_backends)}"
                )
    ...
    def _check_solver_health(self, include_details: bool = True) -> HealthStatus:
        ...
        try:
            ...
            if include_details:
                ...
                details = {
                    "default_method": (
                        config.solver.default_method if config else "newton"
                    ),
                    "monitoring_enabled": (
                        config.solver.enable_monitoring if config else True
                    ),
                    "timeout_configured": (
                        config.solver.timeout_minutes is not None if config else False
                    ),
                }
    ...
    def _check_optimization_health(self, include_details: bool = True) -> HealthStatus:
        ...
        try:
            ...
            if include_details:
                ...
                details = {
                    "default_method": (
                        config.optimization.default_method if config else "L-BFGS-B"
                    ),
                    "checkpointing_enabled": (
                        config.optimization.enable_checkpointing if config else True
                    ),
                    "checkpoint_dir_exists": (
                        os.path.exists(config.optimization.checkpoint_dir)
                        if config
                        else False
                    ),
                }
    ...
    def get_health_history(self, hours: int = 24) -> List[SystemHealth]:
        ...
        with self._lock:
            return [
                ...
                if datetime.fromisoformat(health.timestamp.replace("Z", "+00:00"))
                > cutoff_time
            ]
                    ...
                    jsonify(
                        {
                            ...
                            "available_components": list(
                                health_checker.component_checkers.keys()
                            ),
                        }
                    ),
                    404,
            ...
            return jsonify(
                {
                    ...
                    - get_global_health_checker().start_time,
                }
            )
