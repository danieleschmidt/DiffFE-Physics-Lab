...
class BackendManager:
    ...
    def refresh_backend_status(self) -> None:
        ...
        # Log status
        available_backends = [name for name, status in self._status_cache.items() if status.available]
    ...
    def _check_torch_status(self) -> BackendStatus:
        ...
        try:
            ...
            # GPU information
            gpu_support = torch.cuda.is_available() if hasattr(torch.cuda, 'is_available') else False
    ...
    def _check_numpy_status(self) -> BackendStatus:
        ...
        try:
            ...
            return BackendStatus(
                name='numpy',
                available=False,
                error_message=f"NumPy not available: {e}"
            )
        except Exception as e:
            return BackendStatus(
                name='numpy',
                available=False,
                error_message=f"NumPy test failed: {e}"
            )
    ...
    def get_best_available_backend(self, prefer_gpu: bool = True) -> Optional[str]:
        ...
        logger.info(f"Selected best backend: {best_backend} (score: {scores[best_backend]})")
        
        return best_backend
    
    def get_backend_with_fallback(self, preferred_backend: str) -> Tuple[Optional[Any], str]:
    ...
    def validate_backend_compatibility(
        ...
        if not status.available:
            return False, [f"Backend '{backend_name}' not available: {status.error_message}"]
        ...
        if min_memory > 0 and status.memory_info:
            if 'gpu_memory_total' in status.memory_info:
                ...
                if available_gb < min_memory:
                    issues.append(f"Insufficient GPU memory: {available_gb:.1f}GB < {min_memory}GB")
        
        # Check device count
        min_devices = requirements.get('min_devices', 1)
        if status.device_count < min_devices:
            issues.append(f"Insufficient devices: {status.device_count} < {min_devices}")
    ...
    def generate_backend_report(self) -> Dict[str, Any]:
        ...
        gpu_backends = [name for name, status in self._status_cache.items() 
    ...
    def grad(self, func: Callable, argnums: int = 0, has_aux: bool = False) -> Callable:
        ...
        def grad_func(*args, **kwargs):
            ...
            if hasattr(x, '__iter__'):
                ...
                # Scalar input
                f_plus = func(*(args[:argnums] + (x + eps,) + args[argnums+1:]), **kwargs)
                f_minus = func(*(args[:argnums] + (x - eps,) + args[argnums+1:]), **kwargs)
        ...
        def jacobian_func(*args, **kwargs):
            ...
            if hasattr(x, '__iter__'):
                ...
                for i in range(input_dim):
                    ...
                    jac[:, i] = [(fp - fm) / (2 * eps) for fp, fm in zip(f_plus, f_minus)]
        ...
        def vmap_func(*args):
            ...
            if in_axes == 0:
                ...
                for i in range(len(args[0])):
                    result = func(*(arg[i] if j == 0 else arg for j, arg in enumerate(args)))
...
def validate_backend_requirements(
    ...
    """
    manager = get_global_backend_manager()
    compatible, issues = manager.validate_backend_compatibility(backend_name, requirements)
