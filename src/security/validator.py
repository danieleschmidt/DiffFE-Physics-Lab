...
class SecurityValidator:
    ...
    def _initialize_dangerous_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "path_traversal": re.compile(
                r"(\.\.[/\\\\])|(/etc/)|(/proc/)|(/sys/)", re.IGNORECASE
            ),
            "ldap_injection": re.compile(
                r"[()!&|*]|(\bAND\b)|(\bOR\b)|(\bNOT\b)", re.IGNORECASE
            ),
        }

    def sanitize_string(
        self, value: str, allow_html: bool = False, max_length: Optional[int] = None
    ) -> str:
    ...
    def validate_json(
        self, json_str: str, max_size: int = 1024 * 1024
