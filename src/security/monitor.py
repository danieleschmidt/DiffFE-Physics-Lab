...
class SecurityMonitor:
    ...
    def add_rate_limit_rule(
        ...
        logger.info(
            f"Added rate limit rule: {rule_name} - {max_requests}/{time_window}s"
        )

    def is_request_allowed(
        self, rule_name: str, client_ip: str, endpoint: Optional[str] = None
    ) -> bool:
    ...
    def _check_brute_force_attack(
        ...
        failed_attempts = [
            ...
            if (
                ...
                and e.event_type in ["login_failed", "authentication_failed"]
            )
        ]
    ...
    def _check_scan_attempts(
        ...
        scan_events = [
            ...
            if (
                e.source_ip == event.source_ip
                and e.event_type in ["404_error", "forbidden_access"]
            )
        ]
    ...
    def generate_security_report(self, days: int = 7) -> Dict[str, Any]:
        ...
        # Top threats
        top_event_types = sorted(event_types.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        top_threat_ips = sorted(
            ip_stats.items(), key=lambda x: x[1]["threat_score"], reverse=True
        )[:10]
