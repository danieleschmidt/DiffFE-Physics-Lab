...
class DiffFEError(Exception):
    ...
    def _log_error(self):
        ...
        if self.error_code.value >= 7000:  # Security errors
            logger.critical(
                f"Security Error [{self.error_code.name}]: {self.message}",
                extra=log_data,
            )
        elif self.error_code.value >= 4000:  # Solver/computation errors
            logger.error(
                f"Computation Error [{self.error_code.name}]: {self.message}",
                extra=log_data,
            )
        else:
            logger.warning(
                f"Framework Error [{self.error_code.name}]: {self.message}",
                extra=log_data,
            )
...
class MeshValidationError(ValidationError):
    ...
    def __init__(
        self, message: str, mesh_info: Optional[Dict[str, Any]] = None, **kwargs
    ):
        context = kwargs.get("context", {})
        if mesh_info:
...
def validate_and_raise(
    condition: bool, message: str, error_class: type = ValidationError, **kwargs
):
...
def handle_solver_failure(
    solver_type: str, iterations: int, residual: float, tolerance: float
):
