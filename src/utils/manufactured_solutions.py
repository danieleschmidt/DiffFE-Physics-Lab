...
def _trigonometric_solution(
    dimension: int, params: Dict[str, Any]
) -> Dict[str, Callable]:
    ...
    if dimension == 1:
        ...
        def gradient(x):
            dudx = (
                amplitude * frequency * np.pi * np.cos(frequency * np.pi * x[0] + phase)
            )
...
def _mixed_solution(dimension: int, params: Dict[str, Any]) -> Dict[str, Callable]:
    ...
    trig_sol = _trigonometric_solution(
        dimension, {"frequency": trig_freq, "amplitude": trig_amp}
    )
...
def create_boundary_conditions_from_mms(
    manufactured_solution: Dict[str, Callable], boundary_markers: Dict[str, Any]
...
def verify_mms_consistency(
    ...
    if test_points is None:
        ...
        test_points = np.array(
            [[0.1, 0.1], [0.5, 0.5], [0.9, 0.9], [0.2, 0.8], [0.8, 0.2]]
        )
...
def trigonometric_2d(frequency: float = 1.0):
    ...
    sol = generate_manufactured_solution(
        SolutionType.TRIGONOMETRIC, 2, {"frequency": frequency}
    )
    return sol["solution"], sol["source"]
...
def exponential_2d(decay_rate: float = 1.0):
    ...
    sol = generate_manufactured_solution(
        SolutionType.EXPONENTIAL, 2, {"decay_rate": decay_rate}
    )
    return sol["solution"], sol["source"]
...
def compute_source_term(solution_func: Callable, point: np.ndarray) -> float:
    """Compute source term for a given solution function at a point."""
