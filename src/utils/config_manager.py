...
class ConfigValidator:
    ...
    def _validate_security_config(self, security_config: SecurityConfig) -> List[str]:
        ...
        if security_config.jwt_expiration_hours < 1:
            errors.append(f"Invalid JWT expiration: {security_config.jwt_expiration_hours}")
        
        if not (4 <= security_config.bcrypt_rounds <= 20):
            errors.append(f"Bcrypt rounds should be 4-20: {security_config.bcrypt_rounds}")
    ...
    def _validate_api_config(self, api_config: APIConfig) -> List[str]:
        ...
        if api_config.max_content_length < 1024:  # At least 1KB
            errors.append(f"Max content length too small: {api_config.max_content_length}")
    ...
    def _validate_optimization_config(self, opt_config: OptimizationConfig) -> List[str]:
        ...
        if opt_config.checkpoint_frequency < 1:
            errors.append(f"Invalid checkpoint frequency: {opt_config.checkpoint_frequency}")
...
class ConfigManager:
    ...
    def load_config(self, config_path: Optional[str] = None) -> DiffHEConfig:
        ...
        self._config = config
        logger.info(f"Configuration loaded successfully from {self._config_file_path or 'defaults and environment'}")
    ...
    def _load_from_environment(self) -> Dict[str, Any]:
        ...
        for env_var, config_path in self.env_mappings.items():
            ...
            if env_value is not None:
                ...
                logger.debug(f"Set config {config_path} from environment variable {env_var}")
    ...
    def create_sample_config(self, output_path: str, format: str = 'json'):
        ...
        sample_config.security.jwt_secret_key = "your-jwt-secret-key-here-min-32-characters"
