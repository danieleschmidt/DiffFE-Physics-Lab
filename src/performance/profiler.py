...
class PerformanceProfiler:
    ...
    def __init__(self, enable_memory_profiling: bool = True, enable_stack_trace: bool = False):
        ...
        self.memory_monitor = MemoryMonitor() if self._is_monitoring_available() else None
        ...
        logger.info(f"Performance profiler initialized (memory={enable_memory_profiling}, stack={enable_stack_trace})")
    ...
    def profile_context(self, name: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            memory_end = self._get_memory_usage() if self.enable_memory_profiling else 0.0
    ...
    def _check_alerts(self, entry: ProfileEntry):
        ...
        # Memory usage alert
        if (self.enable_memory_profiling and 
            abs(entry.memory_delta) > self.alert_thresholds['memory_usage']):
            alerts.append({
    ...
    def get_recent_entries(self, name: Optional[str] = None, limit: int = 100) -> List[ProfileEntry]:
    ...
    def get_slowest_calls(self, limit: int = 10) -> List[ProfileEntry]:
        ...
        """
        with self._lock:
            sorted_entries = sorted(self.entries, key=lambda e: e.duration, reverse=True)
    ...
    def get_memory_intensive_calls(self, limit: int = 10) -> List[ProfileEntry]:
        ...
        with self._lock:
            sorted_entries = sorted(
                self.entries, 
                key=lambda e: abs(e.memory_delta), 
                reverse=True
            )
            return sorted_entries[:limit]
        ...
        with self._lock:
            ...
            if self.enable_memory_profiling:
                ...
                memory_stats = {
                    ...
                    'avg_memory_per_call': total_memory / total_entries if total_entries > 0 else 0
                }
            ...
            report = {
                'summary': {
                    ...
                    'avg_call_time': total_time / total_entries if total_entries > 0 else 0
                },
                'top_functions_by_time': top_functions,
                'function_statistics': {name: {
                ...
                'slowest_calls': [
                    {
                        'name': e.name,
                        'duration': e.duration,
                        'memory_delta': e.memory_delta if self.enable_memory_profiling else None,
