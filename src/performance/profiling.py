            ...
            try:
                ...
                for thread_id, frame_obj in frame.items():
                    ...
                    while current_frame:
                        ...
                        stack.append(f"{os.path.basename(filename)}:{function_name}:{line_number}")
...
class DetailedProfiler:
    ...
    def __init__(self):
        ...
        logger.info(f"Detailed profiler initialized with backends: {list(self.available_profilers.keys())}")
    ...
    def profile_function(
        ...
        **kwargs
        ...
        cpu_time = (end_cpu_time.user - start_cpu_time.user) + (end_cpu_time.system - start_cpu_time.system)
        ...
        logger.info(f"Profiled {function_name}: {total_time:.4f}s, {memory_delta:.2f}MB, {cpu_usage:.1f}% CPU")
        ...
        class ProfileContext:
            ...
            def __exit__(self, exc_type, exc_val, exc_tb):
                ...
                if self.enable_flamegraph:
                    ...
                    flamegraph_data = self.profiler.flamegraph_generator.generate_flamegraph_data()
                else:
                    flamegraph_data = None
                ...
                logger.info(f"Context profiled '{self.context_name}': {total_time:.4f}s")
    ...
    def save_profile_report(self, output_dir: str, include_flamegraphs: bool = True):
        ...
        for func_name, result in self.profile_results.items():
            ...
            if include_flamegraphs and result.flamegraph_data:
                ...
                temp_generator.save_flamegraph(str(flamegraph_path), f"Profile: {func_name}")
...
class HotspotDetector:
    ...
    def analyze_profile(self, profiler: cProfile.Profile) -> List[PerformanceHotspot]:
        ...
        for (filename, line_number, function_name), (cc, nc, tt, ct) in stats.get_stats().items():
            ...
            if time_percentage >= self.time_threshold_percent or cc >= self.calls_threshold:
...
def profile_performance(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if save_results:
                profiler.save_profile_report(output_dir, include_flamegraphs=enable_flamegraph)
