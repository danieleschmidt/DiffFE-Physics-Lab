...
class ScalingStrategy(ABC):
    ...
    def get_target_workers(
        self, current_workers: int, metrics: PerformanceMetrics
    ) -> int:
...
class AdaptiveScalingStrategy(ScalingStrategy):
    ...
    def should_scale_up(self, metrics: PerformanceMetrics) -> bool:
        ...
        return (
            metrics.cpu_usage > self.cpu_threshold_up
            or metrics.queue_depth > self.queue_threshold_up
        ) and metrics.memory_usage < self.memory_threshold_up

    def should_scale_down(self, metrics: PerformanceMetrics) -> bool:
        """Scale down if CPU low and queue empty."""
        return (
            metrics.cpu_usage < self.cpu_threshold_down
            and metrics.queue_depth == 0
            and metrics.active_workers > self.min_workers
        )
...
class AutoScalingManager:
    ...
    def __init__(
        self, strategy: ScalingStrategy = None, monitoring_interval: float = 10.0
    ):
    ...
    def _scaling_loop(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                target_workers = self.strategy.get_target_workers(
                    current_workers, metrics
                )
...
class MemoryOptimizer:
    ...
    def __init__(self, max_memory_mb: int = None):
        self.max_memory_mb = max_memory_mb or (
            psutil.virtual_memory().total // (1024**2) * 0.8
        )
        self.memory_pools = {}
...
class AdaptiveLoadBalancer:
    ...
    def update_worker_stats(self, worker_id: str, response_time: float, success: bool):
        ...
        if worker_id in self.worker_stats:
            ...
            stats["avg_response_time"] = (1 - alpha) * stats[
                "avg_response_time"
            ] + alpha * response_time
    ...
    def _fastest_response(self) -> str:
        ...
        return min(
            self.workers, key=lambda w: self.worker_stats[w]["avg_response_time"]
        )
