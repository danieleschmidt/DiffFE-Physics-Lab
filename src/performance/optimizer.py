...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Optional[OptimizationConfig] = None):
        ...
        logger.info(
            f"Performance optimizer initialized (threads={self.config.max_threads}, processes={self.config.max_processes})"
        )
    ...
    def _initialize_pools(self):
        ...
        try:
            if self.config.enable_thread_pool:
                ...
                logger.info(
                    f"Thread pool initialized with {self.config.max_threads} workers"
                )
            ...
            if self.config.enable_process_pool:
                ...
                logger.info(
                    f"Process pool initialized with {self.config.max_processes} workers"
                )
    ...
    def _parallel_execution(self, func: Callable, *args, **kwargs):
        ...
        if (
            ...
            and len(args[0]) > self.config.batch_size
        ):
            data = args[0]
            ...
            try:
                ...
                logger.warning(
                    f"Parallel execution failed, falling back to serial: {e}"
                )
                return func(*args, **kwargs)
    ...
    def _create_cache_key(self, func: Callable, args: Tuple, kwargs: Dict) -> str:
        ...
        try:
            ...
            for k, v in sorted(kwargs.items()):
                key_parts.append(
                    f"{k}={hash(v) if hasattr(v, '__hash__') else str(v)[:50]}"
                )
    ...
    def parallel_map(
        ...
        try:
            ...
            timeout = (
                self.config.process_pool_timeout
                if use_processes
                else self.config.thread_pool_timeout
            )
    ...
    def optimize_memory_usage(self, target_mb: Optional[float] = None):
        ...
        if current_memory > target:
            logger.info(
                f"Memory usage {current_memory:.1f}MB exceeds target {target:.1f}MB, optimizing..."
            )
            ...
            logger.info(
                f"Memory optimized: {current_memory:.1f}MB -> {new_memory:.1f}MB"
            )
    ...
    def detect_bottlenecks(self) -> List[Dict[str, Any]]:
        ...
        if bottlenecks:
            ...
            for bottleneck in bottlenecks[:5]:  # Log top 5
                logger.warning(
                    f"  {bottleneck['type']}: {bottleneck['task']} ({bottleneck['severity']})"
                )
    ...
    def get_metrics(self) -> PerformanceMetrics:
        ...
        with self._lock:
            ...
            if self._task_history:
                ...
                self.metrics.throughput = len(recent_tasks) / max(
                    avg_time * len(recent_tasks), 1.0
                )
            ...
            self.metrics.bottleneck_analysis = {
                ...
                "high_severity": len(
                    [b for b in self._bottlenecks if b["severity"] == "high"]
                ),
                "medium_severity": len(
                    [b for b in self._bottlenecks if b["severity"] == "medium"]
                ),
            }
    ...
    def generate_optimization_report(self) -> Dict[str, Any]:
        ...
        # Top tasks by time
        top_tasks = sorted(
            task_summary.items(), key=lambda x: x[1]["total_time"], reverse=True
        )[:10]
    ...
    def _generate_recommendations(
        self, bottlenecks: List[Dict], task_summary: Dict
    ) -> List[str]:
        ...
        if len(self._optimization_cache) < 100:
            recommendations.append(
                "Cache is underutilized - consider enabling caching for more functions"
            )

        # Batch size optimization
        large_tasks = [
            name for name, data in task_summary.items() if data["count"] > 1000
        ]
        if large_tasks:
            recommendations.append(
                f"Consider increasing batch size for high-frequency tasks: {', '.join(large_tasks[:3])}"
            )
...
def parallel_map(
    ...
    """
    return get_global_optimizer().parallel_map(
        func, iterable, chunk_size, use_processes
    )
