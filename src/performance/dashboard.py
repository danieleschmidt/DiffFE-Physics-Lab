...
class MetricsCollector:
    ...
    def _update_aggregated_metrics(self, metric_name: str):
        ...
        self.aggregated_metrics[metric_name] = {
            ...
            "count": len(values),
            "trend": (
                self._calculate_trend(values[-20:]) if len(values) >= 20 else "stable"
            ),
        }
    ...
    def get_metric_history(self, metric_name: str, hours: int = 1) -> List[Dict]:
        ...
        with self.collection_lock:
            ...
            return [
                m
                for m in self.metrics_store[metric_name]
...
class WebDashboard:
    ...
    def _setup_routes(self):
        ...
        def get_alert_history():
            ...
            history = [
                asdict(alert) for alert in self.alert_manager.get_alert_history(hours)
            ]
            return jsonify(history)
...
"""
    ...
    def record_metric(
        ...
        metric = DashboardMetric(
            name=name, value=value, unit=unit, timestamp=time.time(), category=category
        )
        self.metrics_collector.record_metric(metric)

    def add_alert_rule(
        self, rule_name: str, condition: Callable, severity: str = "warning"
    ):
...
def record_dashboard_metric(
    name: str, value: float, unit: str = "", category: str = "general"
):
...
def add_dashboard_alert_rule(
    rule_name: str, condition: Callable, severity: str = "warning"
):
...
def start_dashboard(host: str = "0.0.0.0", port: int = 8080, background: bool = True):
    ...
    dashboard.add_alert_rule(
        "high_cpu", lambda m: m.name == "cpu_percent" and m.value > 80, "warning"
    )

    dashboard.add_alert_rule(
        "high_memory", lambda m: m.name == "memory_percent" and m.value > 90, "critical"
    )
