...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Performance monitor initialized (interval={monitoring_interval}s, history={history_size})"
        )
    ...
    def _check_application_alerts(self, metrics: ApplicationMetrics):
        ...
        if metrics.response_time_avg > self.alert_thresholds.get(
            ...
            alerts.append(
                PerformanceAlert(
                    ...
                    level=(
                        AlertLevel.WARNING
                        if metrics.response_time_avg < 2000.0
                        else AlertLevel.CRITICAL
                    ),
    ...
    def _trigger_alert(self, alert: PerformanceAlert):
        ...
        # Log alert
        log_func = (
            logger.critical if alert.level == AlertLevel.CRITICAL else logger.warning
        )
        ...
        try:
            with self._lock:
                ...
                if self.system_metrics_history:
                    ...
                    self._trend_data["memory_percent"].append(
                        latest_system.memory_percent
                    )
                    self._trend_data["disk_usage_percent"].append(
                ...
                if self.application_metrics_history:
                    ...
                    self._trend_data["response_time_avg"].append(
                        latest_app.response_time_avg
                    )
                ...
                for trend_name in self._trend_data:
                    if len(self._trend_data[trend_name]) > max_trend_size:
                        self._trend_data[trend_name] = self._trend_data[trend_name][
                            -max_trend_size:
                        ]
    ...
    def log_application_metric(
        self, metric_name: str, value: float, aggregator: Optional[Callable] = None
    ):
    ...
    def get_current_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            current_app = (
                self.application_metrics_history[-1]
                if self.application_metrics_history
                else None
            )
    ...
    def get_metrics_history(self, hours: int = 1) -> Dict[str, List[Dict]]:
        ...
        with self._lock:
            ...
            return {"system": system_history, "application": app_history}
    ...
    def _generate_performance_recommendations(
        self, current_metrics: Dict, recent_alerts: List
    ) -> List[str]:
        ...
        if memory_percent > 85:
            recommendations.append(
                f"High memory usage ({memory_percent:.1f}%) - consider memory optimization or scaling"
            )
        ...
        if (
            "cpu_percent" in self._trend_data
            and len(self._trend_data["cpu_percent"]) > 10
        ):
    ...
    def _assess_system_health(self, current_metrics: Dict, recent_alerts: List) -> str:
        ...
        if critical_issues > 0 or critical_alerts > 5:
            ...
            system_metrics.get("cpu_percent", 0) > 70
            or system_metrics.get("memory_percent", 0) > 80
        ):
            return "warning"
        else:
            return "healthy"
