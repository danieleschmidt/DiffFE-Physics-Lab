    ...
    **kwargs
    ...
    def forward_assembly(self, trial: Any, test: Any, params: Dict[str, Any]) -> Any:
        ...
        if self.formulation == "time_harmonic":
            ...
            weak_form = (
                fd.inner(fd.curl(trial), fd.curl(test)) * fd.dx
    ...
    def manufactured_solution(self, **kwargs) -> Dict[str, Callable]:
        ...
        **kwargs
        ...
        if self.formulation == "electrostatic" and dimension == 2:
            ...
            raise ValueError(
                f"Manufactured solution not implemented for {self.formulation} in {dimension}D"
            )
    ...
    def compute_energy(self, field: Any, params: Dict[str, Any] = None) -> float:
        ...
        if self.formulation == "electrostatic":
            ...
            energy = (
                0.5
                * eps
                * fd.assemble(fd.inner(fd.grad(field), fd.grad(field)) * fd.dx)
            )
            ...
            magnetic_energy = (
                0.5
                / mu_param
                * fd.assemble(fd.inner(fd.curl(field), fd.curl(field)) * fd.dx)
            )
