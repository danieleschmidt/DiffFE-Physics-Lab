...
def get_operator(name: str) -> type:
    ...
    if name not in _OPERATOR_REGISTRY:
        raise KeyError(
            f"Operator '{name}' not found. Available: {list(_OPERATOR_REGISTRY.keys())}"
        )
...
class BaseOperator(ABC):
    ...
    **kwargs
    ...
    def __call__(self, trial: Any, test: Any, params: Dict[str, Any] = None) -> Any:
        """Apply the operator.
    ...
    def validate_inputs(self, trial: Any, test: Any, params: Dict[str, Any]) -> None:
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(backend={self.backend}, params={self.params})"
        )
...
class NonlinearOperator(BaseOperator):
    ...
    def linearize(self, solution: Any, params: Dict[str, Any] = None) -> LinearOperator:
    ...
    def newton_step(self, current_solution: Any, params: Dict[str, Any] = None) -> Any:
