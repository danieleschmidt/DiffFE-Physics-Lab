...
from ..utils.validation import validate_boundary_conditions, validate_function_space
...
class Problem:
    ...
    def __init__(self, mesh=None, function_space=None, backend: str = "numpy"):
    ...
    def _assemble_boundary_conditions(self) -> List:
        ...
        for bc_name, bc_def in self.boundary_conditions.items():
            if bc_def["type"] == "dirichlet":
                ...
                bc = fd.DirichletBC(self.function_space, value, bc_def["boundary"])
                bcs.append(bc)
        return bcs
    ...
    def compute_gradient(self, objective: Callable, parameters: List[str]) -> Callable:
        ...
        if self.backend_name == "jax" and HAS_JAX:
            ...
            raise NotImplementedError(
                f"Gradient computation not supported for backend {self.backend_name}"
            )
    ...
    def optimize(
        ...
        # Run optimization
        result = minimize(
            scipy_objective, x0, method=method, jac=jac, options=options or {}
        )
    ...
    def generate_observations(
        self, num_points: int, noise_level: float = 0.0, seed: int = None
    ) -> np.ndarray:
