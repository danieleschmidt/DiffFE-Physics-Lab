...
class ComplianceManager:
    ...
    def _validate_processing(
        self, record: DataProcessingRecord, framework: ComplianceFramework
    ):
    ...
    def _validate_gdpr(self, record: DataProcessingRecord) -> List[str]:
        ...
        if (
            record.retention_period and record.retention_period > 2555
        ):  # 7 years max for most data
        ...
        if record.cross_border_transfers:
            violations.append(
                "GDPR: Cross-border transfer requires adequacy decision or appropriate safeguards"
            )
        ...
        if any(cat in record.data_categories for cat in sensitive_categories):
            if record.legal_basis not in [
                ...
                violations.append(
                    "GDPR: Special category data requires specific legal basis"
                )
    ...
    def _validate_ccpa(self, record: DataProcessingRecord) -> List[str]:
        ...
        if "sale" in record.purpose.lower() and record.user_consent is not True:
            violations.append(
                "CCPA: Sale of personal information requires opt-in consent"
            )
    ...
    def _validate_pdpa(self, record: DataProcessingRecord) -> List[str]:
        ...
        # Check consent requirement
        if record.user_consent is not True and record.legal_basis not in [
            "contract",
            "legal_obligation",
        ]:
    ...
    def _record_compliance_violation(
        self, activity_id: str, framework: ComplianceFramework, violations: List[str]
    ):
        ...
        for violation in violations:
            logger.warning(
                f"Compliance violation ({framework.value}): {violation} in activity {activity_id}"
            )
    ...
    def _log_processing(self, record: DataProcessingRecord):
        ...
        if self.config.anonymize_logs:
            log_data["activity_id"] = hashlib.sha256(
                record.activity_id.encode()
            ).hexdigest()[:16]
    ...
    def record_user_consent(
        ...
        logger.info(
            f"Consent recorded: user={user_id}, purpose={purpose}, given={consent_given}"
        )
    ...
    def cleanup_expired_data(self) -> int:
        ...
        self.processing_records = [
            ...
            < timedelta(days=record.retention_period or self.config.data_retention_days)
        ]
    ...
    def generate_compliance_report(self) -> Dict[str, Any]:
        ...
        active_consents = len(
            [
                ...
                if r["consent_given"]
                and not r["withdrawn"]
                and current_time < r["expiry_date"]
            ]
        )
        ...
        for violation in recent_violations:
            ...
            report["compliance_violations"]["by_framework"][framework] += len(
